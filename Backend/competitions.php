...
if (!$con) { die('Could not connect: ' . mysqli_error()); }
  ...
  $var = [];
    ...
    $sql2 = 'SELECT
        ...
        SUM(sustainedinjuries_2 ) + SUM(sustaineddead_2) - SUM(sustainedinjuries_1) - SUM(sustaineddead_1) AS S,
        SUM( case when score_1 > score_2 then 3 else 0 end
            + case when score_1 = score_2 then 1 else 0 end
        ) AS Pts
        FROM (
        SELECT site_matchs.id AS m, site_teams.id AS id, site_teams.teamlogo AS logo, site_teams.name AS team, site_coachs.name AS coach, score_1, score_2, sustainedinjuries_1, sustainedinjuries_2, sustaineddead_1, sustaineddead_2 FROM site_matchs
        ...
        UNION
        SELECT site_matchs.id AS m, site_teams.id AS id, site_teams.teamlogo AS logo, site_teams.name AS team, site_coachs.name AS coach, score_2, score_1, sustainedinjuries_2, sustainedinjuries_1, sustaineddead_2, sustaineddead_1 FROM site_matchs
