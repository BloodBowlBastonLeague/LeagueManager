LeagueManager.directive('admin', function(){
	return {
      ...
      $rootScope.title = "Tout sur la ligue";
			...
			//Saison 2017
			$scope.testing = function(league,competition){
				$http.get('http://web.cyanide-studio.com/ws/bb2/contests/?key=' + window.Cyanide_Key + '&competition=' + competition + '&status=played&league=' + league ).success(function(result){
					...
					for(i=0;i<$scope.savingMatches.length;i++){
								var contest=$scope.savingMatches[i].contest_id;
								//Call Cyanide API
								$http.get('http://web.cyanide-studio.com/ws/bb2/match/?key=' + window.Cyanide_Key + '&uuid=' + $scope.savingMatches[i].match_uuid).success(function(result){

									result.contest_id = contest;
									//Save Match
									$http.post('Backend/match_save_2017.php',result).then( function(result){console.log(result);});
