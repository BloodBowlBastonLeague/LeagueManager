LeagueManager.directive('match1', function() {
	return {
		restrict: 'E',
		templateUrl: '../views/frames/match.html',
		controller: function($scope, $rootScope, $http, $timeout, $routeParams) {
			...
			$http.post("../backend/routes.php?action=match", [$scope.matchID])
				.success(function(result) {
					...
					for (i = 0; i < 2; i++) {
						$scope.teamIdx = $scope.tmpTeams.map(function(e) {
								return e.teamID;
							})
						...
						for (j = 0; j < $rootScope.match.teams[i].popularitybeforematch; j++) {
					...
					$rootScope.match.started = $scope.match.started.replace(/-/g, "")
						.replace(/ /g, "T") + "Z";
					...
					$scope.stadiumBG = {
						'background': 'url("../resources/img/stadium/' + $rootScope.match.stadium + '.png") center center no-repeat, rgba(50, 50, 50, 0.85) ',
